...
def build_embeds(s: dict) -> list[dict]:
    ...
    mod = (
        ...
        "`.p [N] some text` — only messages **containing** that text "
        "(case-insensitive; N defaults to your cap)\n"
        "`.p [N] !some text` — only messages **NOT** containing it\n"
        "`.p @user all` *(admin)* — sweep that user's **entire** history in the channel\n"
        "`.p all` *(admin)* — sweep the **whole channel**; asks for a ✅ confirm "
        "first (30s) and keeps pinned messages. Add text to scope it: `.p all reached`\n"
        "Filters combine with @user, and every prune is logged with the filter used.\n"
    ...
    xp = (
        "Nadeko (prefix `;`) awards **2 XP per message** (75 s cooldown) and "
        "**3 XP/min in voice** — voice levels you ~2× faster by design. Rank "
        ...
        "previous rank (**30 min** for the second rung, scaling to **14 days** "
        "at the top). Reach the XP early and the promotion is deferred — the "
        "member is DMed and the role lands automatically the moment tenure is "
        "served.\n\n"
        "`..level` (`..lvl`, `..rank`, `..next`) — anyone can pull a card with "
        "their avatar, level, total XP, next rank, XP to go (voice/text "
        "estimates), and the live tenure countdown.\n\n"
    ...
    autom = (
        ...
        "self-delete.\n"
        + ("**📚 Reaction role** — #rules-and-info: 📚 book club (the only self-assign ping)\n"
